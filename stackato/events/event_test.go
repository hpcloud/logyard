...
func init() {
	Cases = []Case{
		// supervisord
		Case{Event{Type: "process_start", Process: "supervisord"},
			`INFO success: memcached_node entered RUNNING state, process has ...`},
		Case{Event{Type: "process_stop", Process: "supervisord"},
			`INFO stopped: mysql_node (terminated by SIGKILL)`},
		Case{Event{Type: "process_exit", Process: "supervisord"},
			`INFO exited: dea (exit status 1; not expected)`},
		// kato
		Case{Event{Type: "kato_action", Process: "kato"},
			`[info] (12339) INVOKE kato start`},
		// cc
...
func TestSampleLogs(t *testing.T) {
	for _, cas := range Cases {
		...
		if event == nil {
			t.Fatalf("no event detected for: %s", cas.SampleLog)
		}
